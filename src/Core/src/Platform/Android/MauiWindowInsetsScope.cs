...
internal sealed class MauiWindowInsetsScope : IDisposable
{
		...
		ref SafeAreaBranchContext context)
	{
		...
		for (int edge = 0; edge < 4; edge++)
		{
			...
			var edgeMask = (SafeAreaEdgeMask)(1 << edge);

			if (isExplicit && region == SafeAreaRegions.None)
			{
				context.ImplicitBoundaryMask |= edgeMask;
				continue;
			}

			if (!isExplicit && (context.ImplicitBoundaryMask & edgeMask) != 0)
			{
				continue;
			}
	...
	struct SafeAreaBranchContext
	{
		...
		public SafeAreaEdgeMask ImplicitBoundaryMask;
	}
