...
public class Issue33000 : ContentPage
{
	...
	private void InitializeUI()
	{
		...
		{
			Text = "Tap a button to test email composition",
		...
		var layout = new VerticalStackLayout
		{
			...
			Children =
			{
				new Label
				{
					Text = "Issue 33000: Email.ComposeAsync - Spark app should receive body and subject",
					FontSize = 16,
				...
				new Label
				{
					Text = "This test verifies that email composition works correctly with Spark and other email apps on Android.",
					FontSize = 12,
		...
		var composeBasicButton = new Button
		{
			Text = "Compose Email (Subject + Body)",
			AutomationId = "ComposeBasicEmailButton",
			Margin = new Thickness(0, 0, 0, 5)
		};
		composeBasicButton.Clicked += OnComposeBasicEmail;

		var composeFullButton = new Button
		{
			Text = "Compose Email (To + Subject + Body)",
			AutomationId = "ComposeFullEmailButton",
			Margin = new Thickness(0, 0, 0, 5)
		};
		composeFullButton.Clicked += OnComposeFullEmail;

		var composeHtmlButton = new Button
		{
			Text = "Compose Email (HTML Body)",
			AutomationId = "ComposeHtmlEmailButton",
			Margin = new Thickness(0, 0, 0, 5)
		};
		composeHtmlButton.Clicked += OnComposeHtmlEmail;

		var composeWithCcBccButton = new Button
		{
			Text = "Compose Email (With Cc/Bcc)",
			AutomationId = "ComposeEmailWithCcBccButton",
			Margin = new Thickness(0, 0, 0, 20)
		};
		composeWithCcBccButton.Clicked += OnComposeEmailWithCcBcc;

		layout.Children.Add(composeBasicButton);
		layout.Children.Add(composeFullButton);
		layout.Children.Add(composeHtmlButton);
		layout.Children.Add(composeWithCcBccButton);
